cat > Hakeem-Forensic-Pro.sh << 'EOF'
#!/bin/bash
# ===================================================
...
analyze_image() {
    ...
    html_path="$HOME/Hakeem-Reports/$html_file"
...
create_facebook_page() {
    ...
    page_path="$HOME/Hakeem-Pages/$page_name"
...
log_to_terminal() {
    ...
    # حفظ في ملف log
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $data_type: $data" >> $HOME/Hakeem-Reports/data_log.txt
}
